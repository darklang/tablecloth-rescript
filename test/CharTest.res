...
let suite = suite("Char", () => {
  open Char
  test("toCode", () => expect(toCode('a')) |> toEqual(Eq.int, 97))
  describe("fromCode", () => {
    test("valid ASCII codes return the corresponding character", () =>
      expect(fromCode(97)) |> toEqual(
        {
          open Eq
          option(char)
        },
  ...
  test("toString", () => expect(toString('a')) |> toEqual(Eq.string, "a"))
  describe("fromString", () => {
    test("one-length string return Some", () =>
      expect(fromString("a")) |> toEqual(
        {
          open Eq
          option(char)
        },
  ...
  describe("toLowercase", () => {
    ...
    test("perserves lowercase characters", () => expect(toLowercase('a')) |> toEqual(Eq.char, 'a'))
  ...
  describe("toUppercase", () => {
    ...
    test("perserves uppercase characters", () => expect(toUppercase('A')) |> toEqual(Eq.char, 'A'))
    test("perserves non-alphabet characters", () =>
  ...
  describe("isDigit", () => {
    testAll(
      ...
      digit => expect(isDigit(digit)) |> toEqual(Eq.bool, true),
    )
    test("returns false for all other characters", () =>
  ...
  describe("isPrintable", () => {
    ...
    test("returns false for non-printable character", () =>
      expect(fromCode(31) |> Option.map(~f=isPrintable)) |> toEqual(
        {
          open Eq
          option(bool)
        },
