...
let () = {
  assert Array.equal(Array.initialize(5, ~f=_ => 0), [0, 0, 0, 0, 0], Int.equal)
  ...
  assert String.equal(String.fromList(list{'A', 'B', 'C'}), "ABC")

  assert List.equal(List.initialize(5, ~f=_ => 0), list{0, 0, 0, 0, 0}, Int.equal)
  ...
  assert Tuple3.equal(Tuple3.make(1, 1, 1), (1, 1, 1), Int.equal, Int.equal, Int.equal)
